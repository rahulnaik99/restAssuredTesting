...
public class ecomWebsite {
        ...
        public void getToken() {
                ...
                loginResponse regLogin = given().log().all().spec(req).body(lr).when().post("api/ecom/auth/login")
                                .then().log()
                ...
                createProductResponse cpr = given().spec(req)
                                .param("productName", "Samsung Galaxy S21")
                                .param("productAddedBy", "689859186f585eb60d6c1532")
                                .param("productCategory", "Electronics").param("productSubCategory", "Mobile Phones")
                                .param("productPrice", "799.99")
                                .param("productDescription", "Latest Samsung Galaxy S21 with 128GB storage")
                                .param("productFor", "Sale")
                                .multiPart("productImage", new File("src/test/resources/image.png"))
                ...
                given().spec(req).body(or).when().post("api/ecom/order/create-order").then().log().all();

                req = new RequestSpecBuilder().setBaseUri("https://rahulshettyacademy.com/").addHeader("Authorization",
                                "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJfaWQiOiI2ODk4NTkxODZmNTg1ZWI2MGQ2YzE1MzIiLCJ1c2VyRW1haWwiOiJrYWxsdUBrYWxpeWEuY29tIiwidXNlck1vYmlsZSI6Njk1OTE1MjIwNywidXNlclJvbGUiOiJjdXN0b21lciIsImlhdCI6MTc1NDgzNTk1MiwiZXhwIjoxNzg2MzkzNTUyfQ.2jav1zETgefJq_oBHI9XqUIrKUjw1omTXOmdmuQ6USc")
                                .setContentType(ContentType.JSON).addPathParam("productId", cpr.getProductId()).build();
                given().spec(req).when().delete("api/ecom/product/delete-product/{productId}").then().log().all()
